    ...
    function ($window, $rootScope, $tooltip) {
      ...
      function TypeaheadFactory(element, controller, config) {
        ...
        scope.$select = function (index, evt) {
          ...
          if (options.limit > 6) {
            scope.onUpdateJobnumber(scope.$matches[index].value);
          }
        };
        scope.$isVisible = function () {
      ...
      link: function postLink(scope, element, attr, controller) {
        ...
        controller.$render = function () {
          ...
          var index = typeahead.$getIndex(controller.$modelValue);
            ...
            if(angular.isObject(selected) && selected.label != undefined){
                selected = angular.isObject(selected) ? selected.label : selected;
            }else if(angular.isObject(selected) && selected.VendorName != undefined){
              
              selected = angular.isObject(selected) ? selected.VendorName : selected;
            }
