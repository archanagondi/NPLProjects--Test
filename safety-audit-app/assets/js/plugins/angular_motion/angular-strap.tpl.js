...
angular.module('mgcrea.ngStrap.alert').run([
  ...
  function ($templateCache) {
    ...
    $templateCache.put('alert/alert.tpl.html', '<div class="msg success" style="height:auto;color: #3c763d;background-color: #dff0d8;border-color: #d6e9c6;" tabindex="-1" ng-class="[type ? \'alert-\' + type : null]">Success ! Image Added <button type="button" class="close" style="padding: 2px;background: none;font-size: 29px;font-weight: 700;line-height: 1;color: #000;text-shadow: 0 1px 0 #fff;filter: alpha(opacity=20);opacity: .2;margin: 0px;border: 0;border-radius: 0;right: -2px;top: -6px;" ng-if="dismissable" ng-click="$hide()">&times;</button></div>');
  }
]);
...
angular.module('mgcrea.ngStrap.aside').run([
  '$templateCache',
  function ($templateCache) {
    $templateCache.put('aside/aside.tpl.html', '<div class="aside" tabindex="-1" role="dialog"><div class="aside-dialog" show-aside template="{{aside_content}}"></div></div>');
  }
]);
...
angular.module('mgcrea.ngStrap.modal').run([
  '$templateCache',
  function ($templateCache) {
    $templateCache.put('modal/modal.tpl.html', '<div class="modal" tabindex="-1" role="dialog"><div class="modal-dialog" show-data template="{{modal_content}}"  style="background:#FFF;"></div></div>');
  }
]);
...
angular.module('mgcrea.ngStrap.typeahead').run([
  '$templateCache',
  function ($templateCache) {
    $templateCache.put('typeahead/typeahead.tpl.html', '<ul tabindex="-1" class="typeahead dropdown-menu" ng-show="$isVisible()" role="select"><li role="presentation" ng-repeat="match in $matches" ng-class="{active: $index == $activeIndex}"  ng-click="$select($index, $event)" ><span role="menuitem" tabindex="-1" ng-bind="match.label"></span></li></ul>');
  }
]);
